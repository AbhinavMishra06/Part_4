...
function preload() {
  ...
  bgStart = loadImage("images/Background/beautiful-alien-landscape-space-background-pink-colors-game-design_212168-55.jpg");
  bgSelect = loadImage("images/Background/ankita-dutta-space.jpg");
...
function draw() {
  ...
  if (gameState ==="serve"){
    ...
    Background.scale = 1.83;

    start = createSprite(683, 475);
    start.addImage(playImg);
    start.scale = 1;
    instructions = createSprite(683, 550);
  ...
  if (gameState === "selectLevel"){
    ...
    easyLevel = createSprite(366, 300);
    easyLevel.addImage(easyLevelImg);
    easyLevel.scale = 0.5
    mediumLevel = createSprite(683, 300);
    mediumLevel.addImage(mediumLevelImg);
    mediumLevel.scale = 0.5
    hardLevel = createSprite(1000, 300);
  ...
  if (gameState === "selectPlane"){
    start.visible = true
    textSize(50)
    text("select plane", 200, 200)
  }
  ...
  if (gameState === "play"){
    ...
    if (powerUpGroup3.isTouching(bulletGroup)) {
      bulletGroup.destroyEach();
      powerUpGroup.destroyEach();
      if (frameCount % 30 === 0){
        powerUpTime = powerUpTime -1;
      }
      if (keyDown("space") && powerUpTime < 0) {
        createBullet();
      }
    }
  ...
  // END
    ...
    fighterPlane1.visible = false;
    fighterPlane2.visible = false;
    fighterPlane3.visible = false;
